...
class SpringMuleContextServiceConfigurator
{
    ...
    private static final ImmutableMap<String, BeanDefinition> DEFAULT_CONTEXT_SERVICES = ImmutableMap.<String, BeanDefinition>builder()
    ...
    {
        DEFAULT_CONTEXT_SERVICES.entrySet().stream().filter(service -> !APPLICATION_ONLY_SERVICES.contains(service.getKey()) || artifactType.equals(APP)).forEach(service -> {
    ...
    {
        registerBeanDefinition(QUEUE_STORE_DEFAULT_PERSISTENT_NAME, getBeanDefinition(DefaultObjectStoreFactoryBean.class, "createDefaultPersistentQueueStore"));
        beanDefinitionRegistry.registerAlias(QUEUE_STORE_DEFAULT_PERSISTENT_NAME, "_fileQueueStore");
        registerBeanDefinition(QUEUE_STORE_DEFAULT_IN_MEMORY_NAME, getBeanDefinition(DefaultObjectStoreFactoryBean.class, "createDefaultInMemoryQueueStore"));
        ...
        OBJECT_STORE_NAME_TO_LOCAL_OBJECT_STORE_NAME.entrySet().forEach(objectStoreLocal -> {
            customizationService.getCustomizedService(objectStoreLocal.getKey()).ifPresent(customService -> {
                ...
                customService.getServiceClass().ifPresent(serviceClass -> {
                    ...
                    beanDefinitionRegistry.registerBeanDefinition(objectStoreLocal.getValue(), DEFAULT_CONTEXT_SERVICES.get(objectStoreLocal.getKey()));
                });
            });
        });
